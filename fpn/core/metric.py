...
class RCNNFGAccuracy(mx.metric.EvalMetric):
    ...
    def update(self, labels, preds):
        pred = preds[self.pred.index('rcnn_cls_prob')]  #[1,128,81]
        if self.e2e:
            label = preds[self.pred.index('rcnn_label')]  #[128,1]
        else:
            label = preds[self.label.index('rcnn_label')]
...
class RPNFGFraction(mx.metric.EvalMetric):
    ...
    def update(self,labels,preds):
        ...
        if self.e2e:
            ...
            label = labels[self.pred.index('rcnn_label')]
        num_classes = pred.shape[-1]
        label = label.asnumpy().reshape(-1,).astype('int32')
...
class RPNLogLossMetric(mx.metric.EvalMetric):
    ...
    def update(self, labels, preds):
        pred = preds[self.pred.index('rpn_cls_prob')]   #[1,2,p]
        label = labels[self.label.index('rpn_label')]
