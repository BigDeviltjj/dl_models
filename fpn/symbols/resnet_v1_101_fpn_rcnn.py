...
def resnet_v1_101_fpn_rcnn(Symbol):
    def __init__(self):
    ...
    def get_fpn_feature(self, c2,c3,c4,c5,feature_dim = 256):
        ...
        fpn_p5 = mx.sym.Convolution(data=fpn_p5_1x1, kernel=(3,3), pad = (1,1), stride = (1,1),num_filter = feature_dim, name = 'fpn_p5')
    ...
    def get_rpn_subnet(self, feat, num_anchor, suffix):
        rpn_conv = mx.sym.Convolution(data= feat,kernel=(3,3),pad=(1,1),num_filter=512,
                                        weights = self.shared_param_dict['rpn_conv_weight'],
        ...
        rpn_cls_score = mx.sym.Convolution(data = rpn_relu,kernel=(3,3),pad=(0,0),stride=(1,1),num_filter = 2*num_anchor,
                                        weights = self.shared_param_dict['rpn_cls_score_weight'],
        ...
        rpn_bbox_pred = mx.sym.Convolution(data = rpn_relu,kernel=(3,3),pad=(0,0),stride=(1,1),num_filter = 4*num_anchor,
                                        weights = self.shared_param_dict['rpn_bbox_pred_weight'],
    ...
    def get_symbol(self, cfg, is_train = True):
        ...
        #rpn_bbox_loss_p2:(1,4A,W*H)
        #rpn_bbox_pred:(1,4A,H,W)
        ...
        if is_train:
            ...
            rois, label, bbox_target, bbox_weight = mx.sym.Custom(rois = rois, gt_boxes = gt_boxes_reshape,op_type = 'proposal_target,'
                                                        num_classes = num_reg_classes, batch_size = cfg.TRAIN.BATCH_IMAGES,
        ...
        fc_new_1_relu = mx.sym.Activation(data=fc_new_1,act_type='relu',name = 'fc_new_2_relu')
        ...
        if is_train:
            ...
            rcnn_label = mx.sym.Reshape(dat=rcnn_label, shape = (cfg.TRAIN.BATCH_IMAGES, - 1),name = 'label_reshape')
